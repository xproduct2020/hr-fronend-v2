...
export default function EmployerDashboard() {
  ...
  async function deleteJob(id) {
    ...
    try {
      const t = localStorage.getItem('token');
      await apiRequest(`/employer/jobs/${id}`, { method: 'DELETE', headers: authHeader(t) });
    ...
    <div style={{ fontFamily: 'sans-serif', maxWidth: 960, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          {dashboard?.employer?.industry && (
            <p style={{ margin: '4px 0 0', color: '#666' }}>{dashboard.employer.industry}</p>
          )}
        </div>
        <button
          onClick={() => { localStorage.removeItem('token'); router.push('/employer'); }}
            ...
            {jobs.map((job) => (
              <tr key={job.id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={{ padding: '8px 12px', whiteSpace: 'nowrap' }}>
                  ...
                  <button
                    onClick={() => router.push(`/employer/job-post?id=${job.id}`)}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: 10, padding: 28, width: '100%', maxWidth: 700, maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0 }}>Applicants — {viewingJob.title}</h3>
              <button onClick={() => setViewingJob(null)} style={{ border: 'none', background: 'none', fontSize: 20, cursor: 'pointer', color: '#666' }}>✕</button>
            </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <select
                      ...
                      onChange={(e) => updateApplicationStatus(viewingJob.id, a.application_id, e.target.value)}
